...
def _date_score(receipt, tx):
    ...
    return (1.0, 0) if delta_days == 0 else ((0.85, 1) if abs(delta_days) == 1 else (0.0, delta_days))
...
def _receipt_card_last4(receipt):
    return _last4((receipt.raw_openai_json or {}).get('payment_card_last4'))


def _bank_card_last4(tx):
    raw_row = tx.raw_row or {}
    explicit = raw_row.get('_payment_card_last4') or raw_row.get('payment_card_last4')
    if explicit:
        return _last4(explicit)
    masked = raw_row.get('_payment_card_masked') or raw_row.get('payment_card_masked')
    if masked:
        return _last4(masked)
    return _last4(tx.raw_description)


def _direct_card_match(receipt, tx):
    receipt_card = _receipt_card_last4(receipt)
    bank_card = _bank_card_last4(tx)
    return bool(receipt_card and bank_card and receipt_card == bank_card)
...
def match_score(receipt, tx):
    receipt_card = _receipt_card_last4(receipt)
    bank_card = _bank_card_last4(tx)
    direct_card_match = _direct_card_match(receipt, tx)
    base_reason = {
        ...
        'receipt_card_last4': receipt_card,
        'bank_card_last4': bank_card,
        'direct_card_match': direct_card_match,
    }
    if tx.amount >= 0:
    ...
    learned_card_bank_match = bool(learned_bank and learned_bank == tx.bank)
    if merchant < MIN_MERCHANT_REVIEW_SCORE and not direct_card_match and not learned_card_bank_match:
    ...
    score = 0.50 + 0.20 * date_score + 0.12 * merchant + 0.05 * payment
    if direct_card_match:
        score += 0.13
    elif learned_card_bank_match:
        score += 0.05
    return min(score, 1.0), {
        **base_reason,
        ...
        'merchant': merchant,
        'payment': payment,
        'learned_card_bank': learned_bank or '',
        'card_history_matches': card_matches,
        'card_bank_share': card_bank_share,
        'learned_card_bank_match': learned_card_bank_match,
    }
...
def match_bank_transactions_for_receipt(receipt):
    ...
    for tx in candidates:
        ...
        if score >= MIN_REVIEW_SCORE:
            status = 'auto_matched' if score >= AUTO_MATCH_SCORE and reason.get('delta_days') == 0 and (reason.get('merchant', 0) >= MIN_MERCHANT_AUTO_SCORE or reason.get('direct_card_match')) else 'needs_review'
