...
def receipts_dashboard(request):
    ...
    # Source-agnostic budget model:
    # - receipt items represent detailed expenses from receipts/factures,
    # - unmatched negative bank transactions represent standalone expenses,
    # - matched bank transactions are not counted again.
    ...
    monthly_receipts = list(receipts.filter(purchased_at__isnull=False).annotate(month=TruncMonth('purchased_at')).values('month').annotate(spent=Sum('items__paid_price'), saved=Sum('items__discount_amount'), count=Count('id')).order_by('-month')[:12])
    monthly_receipts = list(reversed(monthly_receipts))
    monthly_rows = attach_bars(monthly_receipts)
    ...
    standalone_count = standalone_expenses.count()
    ...
    context = {
        **admin.site.each_context(request),
        ...
        'recent_uncategorized': standalone_expenses.filter(category__in=['', 'inne']).order_by('-transaction_at', '-booked_at', '-id')[:8],
